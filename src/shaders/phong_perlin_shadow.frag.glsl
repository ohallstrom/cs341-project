...
void main() {
    ...
    if (dot(dir_to_light, N) > 0.){
        int_diff = diff_color * (light_color)* dot(N, dir_to_light); 
        vec3 r = reflect(dir_to_light, N);
        if (dot(N,dir_to_light)>0. && dot(r, dir_from_view)>0.) {
            int_spec = spec_color *(light_color) * pow(dot(r, dir_from_view), shininess);
        } 
    }
    ...
    if (shadow_dist*1.01> dist){
        color += scale_value*(int_spec + int_diff);
    }
